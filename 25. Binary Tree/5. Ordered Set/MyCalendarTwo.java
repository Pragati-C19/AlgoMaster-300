...
public class MyCalendarTwo {
    ...
    public boolean book(int startTime, int endTime) {
        ...
        for(int[] event : overlapingEvents){
            int overlapStart = event[0];
            int overlapEnd = event[1];

            // Add overlap only if it's not already present in doubleBookingEvents
            if (!doubleBookingEvents.containsKey(overlapStart)) {
